...
target_col = 'flood' if 'flood' in df.columns else 'is_flood_event'
...
discriminative_features = sorted(
    ...
    key=lambda x: abs(x[1]['separation_score']
                      ) if x[1]['separation_score'] is not None else 0,
    reverse=True
)[:10]
...
plt.title('Distribution of Flood Events in Dataset',
...
plt.tight_layout()
plt.savefig(VIZ_DIR / '01_class_distribution.png',
...
fig.suptitle('Feature Distributions: Flood vs Non-Flood',
...
for idx, (col, _) in enumerate(discriminative_features[:6]):
    ...
    ax.hist(non_flood_data, bins=20, alpha=0.6,
            label='Non-Flood', color='#3498db', edgecolor='black')
    ax.hist(flood_data, bins=20, alpha=0.6, label='Flood',
    ...
    ax.grid(alpha=0.3)
plt.tight_layout()
plt.savefig(VIZ_DIR / '02_feature_distributions.png',
            dpi=300, bbox_inches='tight')
plt.close()
viz_count += 1
...
plt.tight_layout()
plt.savefig(VIZ_DIR / '03_correlation_heatmap.png',
            dpi=300, bbox_inches='tight')
plt.close()
viz_count += 1
...
valid_correlations = [(col, corr)
                      for col, corr in sorted_correlations if not np.isnan(corr)]
top_10_features = [col for col, _ in valid_correlations[:10]]
top_10_corrs = [corr for _, corr in valid_correlations[:10]]
colors_fi = ['#e74c3c' if corr > 0 else '#3498db' for corr in top_10_corrs]
plt.barh(top_10_features, top_10_corrs,
         color=colors_fi, alpha=0.7, edgecolor='black')
plt.xlabel('Correlation with Flood Events', fontsize=12)
plt.title('Top 10 Features Correlated with Flooding',
          fontsize=14, fontweight='bold')
...
plt.tight_layout()
plt.savefig(VIZ_DIR / '04_feature_importance.png',
            dpi=300, bbox_inches='tight')
plt.close()
viz_count += 1
...
fig, axes = plt.subplots(2, 3, figsize=(15, 10))
fig.suptitle('Feature Distributions: Box Plots Comparison',
             fontsize=16, fontweight='bold')
for idx, (col, _) in enumerate(discriminative_features[:6]):
    ax = axes[idx // 3, idx % 3]
    data_to_plot = [non_flood_samples[col].dropna(),
                    flood_samples[col].dropna()]
    bp = ax.boxplot(data_to_plot, labels=[
                    'Non-Flood', 'Flood'], patch_artist=True)
    # Color boxes
...
if len(valid_features) >= 2:  # Need at least 2 features for scatter matrix
    ...
    scatter_df['Class'] = scatter_df[target_col].map(
        {0: 'Non-Flood', 1: 'Flood'})
    scatter_df = scatter_df.dropna()  # Ensure no NaNs
    ...
    plt.tight_layout()
    plt.savefig(VIZ_DIR / '06_scatter_matrix.png',
                dpi=300, bbox_inches='tight')
    plt.close()
    viz_count += 1
else:
...
table_data.append(['Feature', 'Mean', 'Std', 'Min', 'Max',
                  'Flood Mean', 'Non-Flood Mean', 'Separation'])
for col, _ in discriminative_features[:10]:
...
plt.title('Statistical Summary: Top 10 Discriminative Features',
          fontsize=14, fontweight='bold', pad=20)
plt.tight_layout()
plt.savefig(VIZ_DIR / '07_statistical_summary.png',
            dpi=300, bbox_inches='tight')
plt.close()
viz_count += 1
...
if missing_pct.max() > 0:
    ...
    plt.xticks(range(len(missing_pct_display)),
               missing_pct_display.index, rotation=45, ha='right')
...
report = {
    ...
    "distribution_analysis": {
        "most_discriminative": [
            {"feature": col, "separation_score": float(
