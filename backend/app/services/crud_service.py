...
from app.models.database_models import User, FloodEvent, Prediction, Recommendation, Feedback
from app.schemas.schemas import UserCreate, UserUpdate, FloodEventCreate, RecommendationCreate
...
class CRUDService:
    ...
    def update_flood_event(db: Session, event_id: int, event_data: Dict[str, Any]) -> Optional[FloodEvent]:
        db_event = db.query(FloodEvent).filter(
            FloodEvent.id == event_id).first()
        if db_event:
    ...
    def update_recommendation(db: Session, rec_id: int, rec_data: Dict[str, Any]) -> Optional[Recommendation]:
        db_rec = db.query(Recommendation).filter(
            Recommendation.id == rec_id).first()
        if db_rec:
