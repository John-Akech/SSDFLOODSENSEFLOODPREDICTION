...
class AlertService:
    ...
    def create_alert(
        ...
        if latitude == -90.0 and longitude == -180.0:
            logger.error(
                "Invalid default coordinates received for alert creation")
            raise ValueError(
                "Invalid coordinates: cannot create alert with default values")
        ...
        if priority_boost:
            ...
            logger.info(
                f"Priority boost applied for {district} ({district_priority} priority): {flood_probability:.2f} → {effective_probability:.2f}")
        ...
        alert = Alert(
            ...
            # Alert expires 12 hours after predicted event
            expires_at=current_time + timedelta(hours=lead_time_hours + 12)
        )

        self.active_alerts[alert.id] = alert
        logger.info(
            f"Created {severity.upper()} alert for {location_str} ({latitude}, {longitude}) - Action: {action}")
    ...
    async def send_web_push_alert(self, alert: Alert, user_subscriptions: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            vapid_subj = settings.VAPID_SUBJECT or "mailto:admin@floodsense.org"
                ...
                try:
                    ...
                    for subscription in user_subscriptions:
                        try:
                            ...
                            logger.warning(
                                f"WebPush failed for {subscription.get('endpoint')}: {e}")
                except Exception as e:
                    logger.warning(
                        f"pywebpush not available or failed: {e}. Falling back to simulate.")
                    for _ in user_subscriptions:
                        await asyncio.sleep(0.05)
                        sent_count += 1
            else:
                logger.warning(
                    "VAPID keys missing – web push delivery is running in simulate mode. "
                    "Populate VAPID_PUBLIC_KEY and VAPID_PRIVATE_KEY (see README Web Push Setup section)."
                )
